...
for (const f of readdirSync(dir).filter(f => f.endsWith('.json'))) {
  ...
  if (f.startsWith('GROUP_')) {
    ...
    for (const r of j.companies) for (const p of ['company_id', 'score', 'band', 'delta_3m', 'trend_6m', 'caja_real', 'last_data_month', 'coverage.erp']) need(r, p, f)
  } else {
    for (const p of ['company_id', 'score.value', 'score.band', 'score.baseline_12m', 'score_series', 'caja_real_series', 'drivers', 'coverage.bank.ok', 'coverage.erp.ok', 'coverage.debt.ok', 'goals']) need(j, p, f)
    ...
    for (const g of j.goals) { need(g, 'plans', f); if (g.plans.length !== 3) { console.error(`${f}: goal ${g.goal_id} has ${g.plans.length} plans, expected 3`); process.exit(1) } }
  }
}
